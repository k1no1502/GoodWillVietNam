...
echo "<h1>🔍 DEBUG VÀ FIX HOÀN TOÀN - SỐ LƯỢNG VẪN 100</h1>";

try {
    // 1. KIỂM TRA CART HIỆN TẠI
    echo "<h2>1. KIỂM TRA CART HIỆN TẠI</h2>";
    $cartItems = Database::fetchAll("SELECT * FROM cart ORDER BY created_at DESC");
    echo "<pre>";
    print_r($cartItems);
    echo "</pre>";
    
    // 2. XÓA TẤT CẢ CART
    echo "<h2>2. XÓA TẤT CẢ CART</h2>";
    Database::execute("DELETE FROM cart");
    echo "<p style='color: green; font-size: 18px;'>✅ ĐÃ XÓA TẤT CẢ CART</p>";
    
    // 3. RESET AUTO_INCREMENT
    echo "<h2>3. RESET AUTO_INCREMENT</h2>";
    ...
    echo "<h2>4. KIỂM TRA INVENTORY</h2>";
    $inventoryItems = Database::fetchAll("SELECT item_id, name, quantity, is_for_sale, price_type FROM inventory WHERE is_for_sale = TRUE AND status = 'available' LIMIT 3");
    echo "<pre>";
    print_r($inventoryItems);
    echo "</pre>";
    
    // 5. THÊM ITEM VÀO CART VỚI QUANTITY = 1
    echo "<h2>5. THÊM ITEM VÀO CART VỚI QUANTITY = 1</h2>";
    ...
    // 6. TẠO CART.PHP MỚI VỚI LOGIC FORCE FIX
    echo "<h2>6. TẠO CART.PHP MỚI VỚI LOGIC FORCE FIX</h2>";
...
// FORCE FIX: Xóa tất cả items có quantity = 100 hoặc quantity > 10
...
// FORCE FIX: Force sửa tất cả items có quantity > available_quantity hoặc quantity = 100
...
<?php include \'includes/footer.php\'; ?>';
    ...
    echo "<p style='color: green; font-size: 18px;'>✅ ĐÃ TẠO CART.PHP MỚI VỚI LOGIC FORCE FIX</p>";
    
    // 7. KIỂM TRA LẠI CART
    echo "<h2>7. KIỂM TRA LẠI CART</h2>";
    $cartItems = Database::fetchAll("SELECT * FROM cart");
    echo "<pre>";
    print_r($cartItems);
    echo "</pre>";
    
    echo "<h1 style='color: green;'>✅ HOÀN THÀNH DEBUG VÀ FIX!</h1>";
    ...
    echo "<li>Mở Developer Tools (F12) và xem Console</li>";
    ...
    echo "<li>Kiểm tra Console (F12) có lỗi gì không</li>";
    echo "<li>Kiểm tra Network tab khi click nút</li>";
    echo "</ul>";
