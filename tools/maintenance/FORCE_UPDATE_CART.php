...
echo "<h1>🔧 FORCE UPDATE CART - CẬP NHẬT CART.PHP</h1>";

try {
    // 1. XÓA TẤT CẢ CART
    echo "<h2>1. XÓA TẤT CẢ CART</h2>";
    Database::execute("DELETE FROM cart");
    echo "<p style='color: green;'>✅ Đã xóa tất cả cart</p>";
    ...
    if ($firstItem) {
        ...
        echo "<p style='color: green;'>✅ Đã thêm item $itemId với quantity = 1</p>";
    }
    
    // 3. KIỂM TRA CART
    echo "<h2>3. KIỂM TRA CART</h2>";
    $cartItems = Database::fetchAll("SELECT * FROM cart");
    echo "<pre>";
    print_r($cartItems);
    echo "</pre>";
    
    // 4. TẠO CART.PHP MỚI VỚI LOGIC SỬA LỖI
    echo "<h2>4. TẠO CART.PHP MỚI VỚI LOGIC SỬA LỖI</h2>";
...
// FORCE FIX: Xóa tất cả items có quantity = 100
Database::execute("DELETE FROM cart WHERE quantity = 100");
...
// FORCE FIX: Sửa tất cả items có quantity > available_quantity hoặc quantity = 100
...
<?php include \'includes/footer.php\'; ?>';
    ...
    echo "<p style='color: green;'>✅ Đã tạo cart.php mới với logic sửa lỗi</p>";
    
    echo "<h1 style='color: green;'>✅ HOÀN THÀNH FORCE UPDATE!</h1>";
